...
/**
 * Above this source length the renderer stops before mermaid: layout of a
 * multi-megabyte diagram blocks the browser thread long enough to look like a
 * hang, and no honest diagram is that large.
 */
...
/**
 * Configure mermaid for the current palette. Reconfiguration is global, so it
 * is memoized on the palette rather than run per render.
 */
function configure(palette: MermaidPalette): void {
  if (configured !== null
...
/** One render attempt's outcome, kept so a focus/theme change can re-run it. */
type RenderState =
...
/**
 * Measure the SVG mermaid wrote into the host, preferring its viewBox because
 * that is the coordinate space the whole diagram lives in.
 ...
 */
function measureIntrinsic(host: HTMLElement | null): IntrinsicSize | null {
...
export function MermaidBody({ content, t, scrollportRef }: DocumentPreviewProps): ReactElement {
  ...
  // and the user's zoom multiplies from there.
  const fitScale = canZoom ? available / intrinsic.width : 1
  // Snap the drawing to the DEVICE pixel grid, not the CSS pixel grid. A whole
  // number of CSS pixels is not a whole number of device pixels: at a 1.25
  // devicePixelRatio a 326px box is 407.5 device pixels, half a pixel off, and
  // the browser resamples the SVG to land on the grid — which reads as blur at
  // exactly the zoom steps where the product is fractional (50% on such a
  // display) and as sharp at the steps where it happens to be whole.
  const devicePixelRatio = typeof window === 'undefined' ? 1 : (window.devicePixelRatio || 1)
  const snapToDeviceGrid = (cssPixels: number): number => {
    const devicePixels = Math.max(1, Math.round(cssPixels * devicePixelRatio))
    return devicePixels / devicePixelRatio
  }
  const renderWidth = canZoom
    ? snapToDeviceGrid(Math.max(1, intrinsic.width * fitScale * zoom))
    : undefined
  const renderHeight = canZoom && renderWidth !== undefined
    ? snapToDeviceGrid(Math.max(1, intrinsic.height * (renderWidth / intrinsic.width)))
    : undefined
